...
function newlevel(level)
{
  ...
  {
    case 0:
      hints.push("[10]Play with S JIKL\nGame by picosonic",
        "[50]Shoot enemies\nwith the honey gun",
...
function intro(percent)
{
  ...
  {
    var tenth=Math.floor(percent/8);
    var curchar=" BEE TYPES ".charAt(tenth);
    ...
    //if ((Math.floor(percent)%16)<=8)
    {
      ...
      write(gs.sctx, (XMAX/6)+(TILESIZE*2), YMAX-15, "KEYS: J=LEFT L=RIGHT I=UP K=DOWN S=SHOOT", 1, textcol);
	  
	 if ((Math.floor(percent)%16)<=8)
      write(gs.sctx, (XMAX/4)+(TILESIZE*2), 10, "PRESS S TO START THE GAME", 1, textcol);
