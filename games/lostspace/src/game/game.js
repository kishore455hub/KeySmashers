...
function draw(){
    switch(gameState){
        case 0:
            ...
            font("LOST IN OUTER SPACE ", 2.5, 0, 5);
            ctx.restore();
            ...
            if(c == 1){
                ...
                ctx.globalAlpha = 0.3;
                ctx.globalAlpha = 1;
                ctx.translate(W / 2 - 50, H /2 + 64);
                font("LOADING.. PRESS ENTER TWICE", 1);
                ctx.restore();
            }
        break;
        case 1:
        ...
        case 3:
            ...
            if(c == 1){
                ...
                ctx.globalAlpha = 0.3;
                ctx.globalAlpha = 1;
                ctx.translate(W / 2, H /2 + 164);
                font("GAME OVER - PRESS ENTER", 1);
                ctx.restore();
				}
...
var shoot= false;
    ...
    if(pressing[32] || pressing[75]){ // Key W
        player[8] = 4;
        var maxVel = 36;
        var forceX = player[4];
        var forceY = player[5];
    ...
    if(pressing[83]){ // Up arrow
        player[8] = 0;
		shoot = true;
    }else
    if(pressing[68]){ // Down arrow
        player[8] = 1;
		shoot = true;
    }else
    if(pressing[65]){ // Left arrow
        player[8] = 2;
		shoot = true;
    }else
    if(pressing[70]){ // Right arrow
        player[8] = 3;
		shoot = true;
    }
	
	
	if (( shoot )  && (t > player[10] && player[8] != 4)){ // Key SPACE
        // SFX
        play(ApShot);
    ...
    // HAL Actions
    if(pressing[71] || pressing[49]){ // Num pad 1 || number 1
