var player = [
    ...
    100,    // 7: Acceleration
    4,      // 8: turret position up, down, left, right, 4 = direction
    10,     // 9: shooter delay
...
var KEY           = { SPACE: 32, LEFT: 74, UP: 73, RIGHT: 76, DOWN: 75 }; // input key codes
...
function drawPlayer(){
    ...
    ctx.lineWidth = 1;
		 ...
		 ctx.font = '18px serif';
		  ctx.fillStyle = '#556B2F';// #7FFF00';
        ctx.fillText('A',300,320);
		ctx.fillText('F',500,320);
		
		ctx.fillText('S',390,230);
		ctx.fillText('D',390,420);
		
		  ctx.fillStyle = '#55FF2F';// #7FFF00';
		
	//	ctx.fillText('G',360,291);
	//	ctx.fillText('H',360,305);
		
		ctx.fillText('K',394,322);
		
		ctx.fillText('J',382,350);
		ctx.fillText('L',408,350);
		
		
        //ctx.fillText(String.fromCharCode(KEY.DOWN),tx(player.rx, tower.ir) + player.collision.ladderDown.x -3, ty(player.ry + player.collision.ladderDown.y)+12);
