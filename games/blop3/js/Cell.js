function Cell(xpos, ypos, radius,aletter) {
	// Inherit from Mover
	this.letter = aletter;
...
function draw_cell(ctx, cam, shadow, player_radius) {
	if (!this.dead) {
		...
		if (this.radius > 4)
		{
					ctx.font = 'italic '+(10+this.radius/3)+'pt Calibri';
		ctx.fillStyle = "black";
	ctx.fillText(this.letter, cam.world_to_viewport_x(this.x_pos)-5, cam.world_to_viewport_y(this.y_pos)); 
