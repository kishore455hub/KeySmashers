var keys="ABCDEFGHIJKLMNOPQRSTUVWXYZ5671234";
var keysx=[];
var keysy=[];
var nextfreeletter="Z";
var gcells =[];
var gcam;
...
function World(canvas) {
	...
	this.load_level = function() {
		...
		gcells.push(new Cell(0, 0, 12,"You!"));
		...
		for (var i=0; i<gcells.length; i++)
			this.level_total_mass += gcells[i].area();
	};
	...
	this.push_player_from = function(x, y) {
		...
		if (player && !player.dead) {
			var dx = x-player.x_pos ;
			var dy = y-player.y_pos ;
			...
			var fx = dx * (3/9);// (400 / (area + 64));
			var fy = dy * (3/9);//(400 / (area + 64));
			...
			var newcell = new Cell(newx, newy, newrad,nextfreeletter);
	...
	this.key_down = function(e) {
		...
		for (var i=0; i<gcells.length; i++) {
				//console.log("comp with code " + gcells[i].letter +" vs " + String.fromCharCode(code));
			if ((gcells[i].letter) ==  String.fromCharCode(code) ) 
			{
				if (!gcells[i].dead) {
					if (gcells[i].radius > 4) {
				console.log("Pressed A key with code " + gcells[i].x_pos +" ," + gcells[i].y_pos);
				
				world.click_at_point(gcam.world_to_viewport_x(gcells[i].x_pos), gcam.world_to_viewport_y(gcells[i].y_pos));
				break;
				}}
			}
...
//			if (delta < 0)
	//			world.cam.scale_target /= 1.2;
		...
		if (code == 32) {
			...
			console.log(world.cam.scale_target);
			if (world.cam.scale_target <0.7) world.cam.scale_target = 5;
			...
			if (!gcells[i].dead) {
				if (!this.paused) {
					for (var j=0; j<gcells.length; j++) {
						if ((i != j) && (!gcells[j].dead)) {
							if (gcells[i].collides_with(gcells[j])) {
								this.transfer_mass(gcells[i], gcells[j]);
							}
						}
					}
					gcells[i].update(this.frame_delta);
				
					// Get some stats about orb sizes
					curr_area = gcells[i].area();
					if (gcells[i].radius > this.get_player().radius) {
					...
					var cell_x = gcells[i].x_pos,
						cell_y = gcells[i].y_pos,
						cellrad = gcells[i].radius,
						dist_from_origin = Math.sqrt(Math.pow(cell_x, 2) + Math.pow(cell_y, 2));
					if (dist_from_origin + cellrad > this.level_radius) {
						// Do some homework
						var cell_xvel = gcells[i].x_veloc,
							cell_yvel = gcells[i].y_veloc;
						
						// Move cell safely inside bounds
						gcells[i].x_pos *= ((this.level_radius-cellrad-1) / dist_from_origin);
						gcells[i].y_pos *= ((this.level_radius-cellrad-1) / dist_from_origin);
						cell_x = gcells[i].x_pos;
						cell_y = gcells[i].y_pos;
						dist_from_origin = Math.sqrt(Math.pow(cell_x, 2) + Math.pow(cell_y, 2));
						...
						gcells[i].x_veloc = cell_speed * Math.cos(new_veloc_ang);
						gcells[i].y_veloc = cell_speed * Math.sin(new_veloc_ang);
				...
				if (i != 0) {
					gcells[i].draw(this.ctx, this.cam, this.shadows, this.get_player().radius);
				}
			}
		}
