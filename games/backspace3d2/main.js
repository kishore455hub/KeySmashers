...
function makeakey(len) {       
                                        
										var text = "";       
										var possibles = "QWERTYUIOPAFHL";    
										var possible ="";     
										var qs="QWERTYUIOPAFGHL";
										
										for (var i = 0; i < possibles.length; i++)  
											if (qs.indexOf(possibles.charAt(i)) != -1) 
												possible = possible + possibles.charAt(i); 
											if (possible=="")       possible="SDF";        
											for (var i = 0; i < len; i++)                 
												text += possible.charAt(Math.floor(Math.random() * possible.length));   
											//      return qs;                                                   
											//      if(document.URL.indexOf("typinggames.zone") == -1) { return "VISIT TYPINGGAMES.ZONE"; };
											return text;                                                             
											}                                                         
...
function updatehud()
{
  ...
  switch (gs.state)
  {
    case 1:
      ...
      writeseg(gs.svg.svghud, 180, 300, "PRESS ENTER TO PLAY NOW!", "gold", 0.7);
      writeseg(gs.svg.svghud, 120, 700, "PLAY WITH KEYS T=SHOOT. MOVE = D J K S", "gold", 0.5);
      break;

    case 2:
      writeseg(gs.svg.svghud, 1000, 50, "SCORE "+gs.score, "gold", 0.5);
	  writeseg(gs.svg.svghud, 540, 370, "D", "gold", 0.5);
	  writeseg(gs.svg.svghud, 620, 300, "J", "gold", 0.5);
	  writeseg(gs.svg.svghud, 620, 200, shootk, "gold", 0.4);
	  writeseg(gs.svg.svghud, 620, 480, "S", "gold", 0.5);
	  writeseg(gs.svg.svghud, 720, 370, "K", "gold", 0.5);
    ...
    case 3:
      if (infectedstatus(1)==0)
      {
        ...
        writeseg(gs.svg.svghud, 185, 480, "THE PLANET IS SAVED", "gold", 0.7);
      }
      else
      {
        writeseg(gs.svg.svghud, 350, 340, "FAILED", "red", 2);
        writeseg(gs.svg.svghud, 140, 480, "TO SAVE THE PLANET", "red", 0.7);
      }
      break;
...
function updatekeystate(e, dir)
{
  ...
  //  case 68: // D
    ...
    case shootk.charCodeAt(0): // enter
   // case 32: // space
	//clear(gs.svg.svghud);
	//writeseg(gs.svg.svghud, 350, 340, "xdX", "red", 2);
