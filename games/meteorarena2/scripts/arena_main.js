...
(function()
{
   ...
   extend(Arena.AttractorScene, Game.Scene,
   {
      ...
      sceneRendererWelcome: function sceneRendererWelcome(ctx)
      {
         ...
         Game.centerFillText(ctx, "Meteor Arena", ff, GameHandler.height*0.5 - 24, "white");
      ...
      sceneRendererInfo: function sceneRendererInfo(ctx)
      {
         ...
         ctx.fillText("Use the keys on screen to target", GameHandler.width * 0.15, ypos);
         ypos += ~~(20 * this.world.scale * 2);
         ctx.fillText("your automatic weapons to that direction.", GameHandler.width * 0.15, ypos);
         ypos += ~~(20 * this.world.scale * 2);
         ctx.fillText("Shoot the enemy ships that will try to destroy you!", GameHandler.width * 0.15, ypos);
         ypos += ~~(20 * this.world.scale * 2);
         ctx.fillText("ESC = PAUSE. SPACEBAR = Sound on/off", GameHandler.width * 0.15, ypos);
      },
...
(function()
{
   ...
   extend(Arena.GameScene, Arena.BaseGameScene,
   {
      ...
      onBeforeRenderScene: function onBeforeRenderScene()
      {
         ...
         if (this.input.axisFire || !(this.game.disableAutofire || (DEBUG && DEBUG.DISABLEAUTOFIRE)) )
         {
            ...
            else
            {
               ...
               var m = new Vector(this.game.mousex , this.game.mousey );
	  ...
	  if (keyCode == GameHandler.KEY.ESC)
            {
               GameHandler.pause();
               return true; 
            }
	  
			this.game.mousex = x[keyCode];
			this.game.mousey = y[keyCode];
			 return true; 
         ...
         switch (keyCode)
         {
            ...
            {
			//this.game.mousex = x[GameHandler.KEY.A];
			//this.game.mousey = y[GameHandler.KEY.A];
...
//               this.input.left = true;
	...
	//this.game.mousex = 250;
    //     this.game.mousey = 250;
			  ...
			  ctx.font = ~~(12 * this.world.scale * 2) + "pt Arial"; 
{
	...
	var yyy1= y1;
		 ...
		 i = GameHandler.KEY.Q;
			x[i] = xxx1 + 100* this.world.scale;
			y[i] = y1 + 200* this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));
		 
		i = GameHandler.KEY.W;
			x[i] = xxx1 + 200* this.world.scale;
			y[i] = y1 + 200* this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));

		 i = GameHandler.KEY.E;
			x[i] = xxx1 + 300* this.world.scale;
			y[i] = y1 + 200* this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));
i = GameHandler.KEY.R;
			x[i] = xxx1 + 400 * this.world.scale;
			y[i] = y1 + 200 * this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));
i = GameHandler.KEY.T;
			x[i] = xxx1 + 500* this.world.scale;
			y[i] = y1 + 200* this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));
i = GameHandler.KEY.Y;
			x[i] = xxx1 + 600* this.world.scale;
			y[i] = y1 + 200* this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));

		 i = GameHandler.KEY.U;
			x[i] = xxx1 + 700* this.world.scale;
			y[i] = y1 + 200* this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));


		 i = GameHandler.KEY.I;
			x[i] = xxx1 + 800* this.world.scale;
			y[i] = y1 + 200* this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));

i = GameHandler.KEY.O;
			x[i] = xxx1 + 900* this.world.scale;
			y[i] = y1 + 200* this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));

i = GameHandler.KEY.P;
			x[i] = xxx1 + 1000* this.world.scale;
			y[i] = y1 + 200* this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));
		 ...
		 var yyy1= y1+150;
		 		 	...
		 		 	i = GameHandler.KEY.A;
			x[i] = xxx1 + 100   *  this.world.scale;
			y[i] = yyy1 + 300   * this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));

		 		 	i = GameHandler.KEY.S;
			x[i] = xxx1 + 200 * this.world.scale;
			y[i] = yyy1 + 250 * this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));
		 	i = GameHandler.KEY.D;
			x[i] = xxx1 + 300 * this.world.scale;
			y[i] = yyy1 + 200 * this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));
		 	
			i = GameHandler.KEY.F;
			x[i] = xxx1 + 400* this.world.scale;
			y[i] = yyy1 + 150* this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));
		 
		 i = GameHandler.KEY.G;
			x[i] = xxx1 + 500* this.world.scale;
			y[i] = yyy1 + 100* this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));
...
xxx1 = xxx1+ 100; 
		 		 i = GameHandler.KEY.Z;
			x[i] = xxx1 + 70* this.world.scale;
			y[i] = yyy1 + 220* this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));

		 i = GameHandler.KEY.X;
			x[i] = xxx1 + 180* this.world.scale;
			y[i] = yyy1 + 250* this.world.scale;		 		
		 ctx.fillText( String.fromCharCode(i) , (x[i]),(y[i]));
i = GameHandler.KEY.C;
			x[i] = xxx1 + 300* this.world.scale;
...
i = GameHandler.KEY.M;
      ...
      renderOverlay: function renderOverlay(ctx)
      {
         ...
         ctx.fillText(sscore, width * 0.2 + width * 0.05, fontYPos);
         ...
         ctx.fillText("HI:" + sscore, width * 0.4 + width * 0.05, fontYPos);
		 
		 //ctx.fillText( "debug "+this.world.scale+" "+(this.game.mousex -GameHandler.offsetX) + " y "+(this.game.mousey -GameHandler.offsetY)  , 250,50	);
		 
		 
         // score multiplier indicator
         ctx.fillText("LEVEL:" + (this.currentWave+1), width * 0.62 + width * 0.1, fontYPos);
